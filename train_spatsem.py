...
checkpoint = '/mnt/sdb1/bnewman1/harpmodels/spatsemnet_2017-12-05_23:48:30.t7'
...
spatsemnet = None
loss_file = '/mnt/sdb1/bnewman1/harplogs/spatsemnet_loss.txt'
pred_file = '/mnt/sdb1/bnewman1/harplogs/spatsemnet_pred.txt'
lowest_loss = 1000000000000
loss_log = None
pred_log = None

if LOAD_FROM_SAVE:
	spatsemnet = SpatSemLSTM()
	params = torch.load(checkpoint)
	spatsemnet.load_state_dict(params)
...
optimizer = optim.Adam(spatsemnet.parameters(), lr=0.001)
...
while True:
	...
	for trial in order:
		...
		for i in range(len(order)):
			...
			"""
			optimizer.zero_grad()
			spatsemnet.sem_hidden = spatsemnet.init_hidden()
			spatsemnet.spat_hidden = spatsemnet.init_hidden()
			print('forward')
			out = spatsemnet(ego_batch, gaze_batch1, gaze_batch2, gaze_batch3)
			print('loss')
			...
			if loss.data[0] < lowest_loss:
				...
				dt = datetime.now().strftime('spatsemnet_%Y-%m-%d_%H:%M:%S')
				torch.save(spatsemnet.state_dict(), '/mnt/sdb1/bnewman1/harpmodels/'+dt+'.t7')
