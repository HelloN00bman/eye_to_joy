...
class SpatSemDataset(Dataset):
	...
	def __getitem__(self, index):
		gaze_data = self.GazeDataset(index)
		joy_data = self.JoyDataset(index)

		egoframe = self.EgoDataset(index)
		gazemap = self.GazeMapDataset(index)

		ego_gaze = torch.zeros(egoframe.shape[0] + gazemap.shape[0], self.w, self.h)
		ego_gaze[0:egoframe.shape[0],:,:] = egoframe
		ego_gaze[egoframe.shape[0]:egoframe.shape[0]+gazemap.shape[0],:,:] = gazemap

		return gaze_data, ego_gaze, joy_data
